...
**This is a skeleton.** The crates compile, build an EIF and measure, but implement no
behaviour. `di-host` and `di-enclave` log and exit non-zero; a skeleton that idled would read
as healthy to a liveness probe. See
...
The host is an ordinary container image. The enclave is a reproducible OCI image and an AWS
Nitro EIF, built by Nix: it converts the OCI root filesystem directly with aws-nitro-util, so
the same commit yields the same PCRs.
...
Release tags and PCR rotation are in [`docs/release.md`](docs/release.md).
