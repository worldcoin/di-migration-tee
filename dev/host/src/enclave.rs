...
/// Bounds how long the single migration slot can be held.
///
/// Generous for an echo, and deliberately so: it is sized for the biometrics pipeline that
/// replaces it. Without a deadline here a wedged enclave would hold the slot forever and every
/// later submission would be refused.
