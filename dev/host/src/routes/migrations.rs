...
use di_dev_api_types::{MIGRATION_CONTENT_TYPE, MigrationAccepted};
...
/// Accepts a compressed PCP and starts a migration.
///
/// Decompression happens here rather than in the background task so a malformed payload is a
/// straight rejection instead of a migration the client has to poll to discover failed.
pub async fn submit(
    ...
    let limit = state.environment().max_pcp_bytes();
    // Decompression is CPU-bound and the payload may be tens of megabytes; on a runtime worker
    // it would stall every other request on this host for its duration.
    let pcp = tokio::task::spawn_blocking(move || compression::decompress(&body, limit))
        .await
        .map_err(|error| {
            AppError::new(
        ...
        .map_err(|error| AppError::payload(&error))?;
    ...
    // The slot guard rides along, so however this task ends the slot is released and the
    // migration settles.
...
pub async fn collect(
    ...
    match migration {
        ...
        MigrationState::Succeeded(pcp) => Ok((
            ...
            .into_response()),
        MigrationState::Failed(failure) => Err(AppError::migration_failure(&failure)),
    }
}
...
mod tests {
    ...
    /// Polls until the migration settles rather than sleeping a fixed interval, so the test does
    /// not depend on how quickly the spawned task is scheduled.
    ...
    async fn a_compression_bomb_is_rejected() {
        ...
        // The state's ceiling is 1 MiB, so this expands past it.
