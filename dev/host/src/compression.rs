...
//!
//! The format is sniffed from the payload rather than taken from a header, so a client does not
//! have to declare what it sent and cannot mislabel it.
...
/// Decompresses `payload`, refusing to hold more than `limit` bytes.
///
/// CPU-bound and unbounded in runtime for a large payload — call it off the async runtime.
///
/// # Errors
///
/// Returns [`Error::UnknownFormat`] when the payload is neither gzip nor zstd,
/// [`Error::TooLarge`] when it expands past `limit`, and [`Error::Corrupt`] when the stream
/// does not decode.
pub fn decompress(payload: &[u8], limit: usize) -> Result<Vec<u8>, Error> {
    ...
    // One byte past the limit, so an over-large payload is caught without ever buffering it.
    // The decoder's own claimed output size is not consulted: that is attacker-controlled.
    let mut pcp = Vec::new();
    reader
        .take(limit as u64 + 1)
        .read_to_end(&mut pcp)
...
mod tests {
    ...
    /// The property that matters: a small payload claiming to expand to gigabytes stops at the
    /// limit instead of being allocated.
    #[test]
